...
class TemporalSelfBlock(nn.Module):
...
class TinyUTAE(nn.Module):
    def __init__(self, in_channels: int = 1, hidden_channels: int = 16, num_heads: int = 4):
        super().__init__()
        ...
        self.temporal_block = TemporalSelfBlock(hidden_channels, num_heads)

        self.query_token = nn.Parameter(torch.zeros(1, 1, hidden_channels))
        self.cross_attn = nn.MultiheadAttention(embed_dim=hidden_channels, num_heads=num_heads, batch_first=True)
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        x = x.permute(0, 3, 4, 1, 2).contiguous().reshape(b * h * w, t, d)  # [BHW,T,D]
        x = self.temporal_block(x)

        q = self.query_token.expand(x.shape[0], -1, -1)
        agg, _ = self.cross_attn(q, x, x, need_weights=False)  # [BHW,1,D]
        agg = agg[:, 0, :]

        agg = agg.reshape(b, h, w, d).permute(0, 3, 1, 2).contiguous()
...
def train_utae_track_o(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_val: np.ndarray,
    y_val: np.ndarray,
    cfg: UTAETrackOConfig,
):
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("UTAE Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_utae_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run UTAE Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = (
        ...
        if args.output_dir
        else base / f"utae_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    print(f"[done] utae synthetic demo saved to: {out}")
...
def utae_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "utae")
    init_kwargs = filter_init_kwargs(TinyUTAE, {"in_channels": int(in_channels), **kwargs})
    model = TinyUTAE(**init_kwargs)
