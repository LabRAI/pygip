...
@dataclass
class EarthFarseerTrackOConfig:
    seq_len: int = 6
    in_channels: int = 1
...
class TinyEarthFarseer(nn.Module):
    def __init__(self, in_channels: int = 1, hidden_channels: int = 16, num_heads: int = 4, num_layers: int = 2):
        ...
        self.far_blocks = nn.ModuleList([TemporalBlock(hidden_channels, num_heads) for _ in range(max(1, num_layers - 1))])
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        # Far-seer branch emphasizes farther temporal gaps.
        x_far = x[:, ::2, :]
        for blk in self.far_blocks:
            x_far = blk(x_far)

        x_last = x_main[:, -1, :] + x_far[:, -1, :]
        x_last = x_last.reshape(b, h, w, d).permute(0, 3, 1, 2).contiguous()
...
def train_earthfarseer_track_o(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_val: np.ndarray,
    y_val: np.ndarray,
    cfg: EarthFarseerTrackOConfig,
):
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("EarthFarseer Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_earthfarseer_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
    ...
    torch.save(
        ...
        output_dir / "earthfarseer_model.pt",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run EarthFarseer Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = (
        ...
        if args.output_dir
        else base / f"earthfarseer_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    print(f"[done] earthfarseer synthetic demo saved to: {out}")
...
def earthfarseer_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "earthfarseer")
    init_kwargs = filter_init_kwargs(TinyEarthFarseer, {"in_channels": int(in_channels), **kwargs})
    model = TinyEarthFarseer(**init_kwargs)
