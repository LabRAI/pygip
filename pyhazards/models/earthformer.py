...
class TinyEarthFormer(nn.Module):
    def __init__(self, in_channels: int = 1, hidden_channels: int = 16, num_heads: int = 4, num_layers: int = 2):
        ...
        self.temporal_blocks = nn.ModuleList(
            [TemporalTransformerBlock(dim=hidden_channels, num_heads=num_heads) for _ in range(num_layers)]
        )
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        x = x[:, -1, :]  # next-step representation per location
        x = x.reshape(b, h, w, d).permute(0, 3, 1, 2).contiguous()  # [B,D,H,W]
...
def train_earthformer_track_o(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_val: np.ndarray,
    y_val: np.ndarray,
    cfg: EarthFormerTrackOConfig,
):
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("Earthformer Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_earthformer_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
    ...
    torch.save(
        ...
        output_dir / "earthformer_model.pt",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Earthformer Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = Path(args.output_dir) if args.output_dir else base / f"earthformer_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"[done] earthformer synthetic demo saved to: {out}")
...
def earthformer_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "earthformer")
    init_kwargs = filter_init_kwargs(TinyEarthFormer, {"in_channels": int(in_channels), **kwargs})
    model = TinyEarthFormer(**init_kwargs)
