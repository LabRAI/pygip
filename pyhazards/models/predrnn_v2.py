...
class SpatioTemporalLSTMCell(nn.Module):
    def __init__(self, in_channels: int, hidden_channels: int):
        ...
        self.conv_x = nn.Conv2d(in_channels, hidden_channels * 7, kernel_size=3, padding=1)
        self.conv_h = nn.Conv2d(hidden_channels, hidden_channels * 4, kernel_size=3, padding=1)
        self.conv_m = nn.Conv2d(hidden_channels, hidden_channels * 3, kernel_size=3, padding=1)

        self.conv_o = nn.Conv2d(hidden_channels * 2, hidden_channels, kernel_size=1)
        self.conv_last = nn.Conv2d(hidden_channels * 2, hidden_channels, kernel_size=1)
    ...
    def forward(
        ...
        x_i, x_f, x_g, x_i_m, x_f_m, x_g_m, x_o = torch.split(x_concat, self.hidden_channels, dim=1)
        h_i, h_f, h_g, h_o = torch.split(h_concat, self.hidden_channels, dim=1)
        m_i, m_f, m_g = torch.split(m_concat, self.hidden_channels, dim=1)

        i_t = torch.sigmoid(x_i + h_i)
        f_t = torch.sigmoid(x_f + h_f + 1.0)
        g_t = torch.tanh(x_g + h_g)
        c_t = f_t * c_prev + i_t * g_t

        i_t_m = torch.sigmoid(x_i_m + m_i)
        f_t_m = torch.sigmoid(x_f_m + m_f + 1.0)
        g_t_m = torch.tanh(x_g_m + m_g)
        m_t = f_t_m * m_prev + i_t_m * g_t_m

        mem = torch.cat([c_t, m_t], dim=1)
        o_t = torch.sigmoid(x_o + h_o + self.conv_o(mem))
...
class TinyPredRNNv2(nn.Module):
    def __init__(self, in_channels: int = 1, hidden_channels: int = 12):
        ...
        self.cell = SpatioTemporalLSTMCell(in_channels=hidden_channels, hidden_channels=hidden_channels)
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        c_state = torch.zeros((b, self.hidden_channels, h, w), device=device)
        m_state = torch.zeros((b, self.hidden_channels, h, w), device=device)

        for t in range(x_seq.shape[1]):
            x_t = self.encoder(x_seq[:, t])
            h_state, c_state, m_state = self.cell(x_t, h_state, c_state, m_state)
...
def train_predrnn_v2_track_o(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_val: np.ndarray,
    y_val: np.ndarray,
    cfg: PredRNNv2TrackOConfig,
):
    ...
    model = TinyPredRNNv2(in_channels=cfg.in_channels, hidden_channels=cfg.hidden_channels).to(device)
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("PredRNN-v2 Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_predrnn_v2_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
    ...
    torch.save(
        ...
        output_dir / "predrnn_v2_model.pt",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PredRNN-v2 Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = Path(args.output_dir) if args.output_dir else base / f"predrnn_v2_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"[done] predrnn_v2 synthetic demo saved to: {out}")
...
def predrnn_v2_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "predrnn_v2")
    init_kwargs = filter_init_kwargs(TinyPredRNNv2, {"in_channels": int(in_channels), **kwargs})
    model = TinyPredRNNv2(**init_kwargs)
