...
class TinyConvLSTM(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            in_ch = enc_channels if i == 0 else hidden_channels
            cells.append(ConvLSTMCell(input_channels=in_ch, hidden_channels=hidden_channels, kernel_size=kernel_size))
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        h_states = [
            torch.zeros((b, self.hidden_channels, h, w), device=device, dtype=x_seq.dtype)
        ...
        for t in range(x_seq.shape[1]):
            x_t = self.encoder(x_seq[:, t])  # [B,enc,H,W]
            for i, cell in enumerate(self.cells):
                h_i, c_i = cell(x_t, h_states[i], c_states[i])
                h_states[i], c_states[i] = h_i, c_i
                x_t = h_i

        return self.decoder(h_states[-1])
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("ConvLSTM Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ConvLSTM Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = (
        ...
        if args.output_dir
        else base / f"convlstm_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    print(f"[done] convlstm synthetic demo saved to: {out}")
...
def convlstm_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "convlstm")
    init_kwargs = filter_init_kwargs(TinyConvLSTM, {"in_channels": int(in_channels), **kwargs})
    model = TinyConvLSTM(**init_kwargs)
