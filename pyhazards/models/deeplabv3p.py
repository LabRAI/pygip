...
class TinyBackbone(nn.Module):
    def __init__(self, in_channels: int = 1, base_channels: int = 16):
        ...
        self.low = nn.Sequential(
            ConvBNReLU(in_channels, c1, kernel_size=3, stride=1),
...
class ASPP(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        super().__init__()
        self.branch1 = ConvBNReLU(in_channels, out_channels, kernel_size=1, stride=1, dilation=1)
        self.branch2 = ConvBNReLU(in_channels, out_channels, kernel_size=3, stride=1, dilation=2)
        self.branch3 = ConvBNReLU(in_channels, out_channels, kernel_size=3, stride=1, dilation=4)
        self.branch4 = ConvBNReLU(in_channels, out_channels, kernel_size=3, stride=1, dilation=6)
        self.project = ConvBNReLU(out_channels * 4, out_channels, kernel_size=1, stride=1, dilation=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        b3 = self.branch3(x)
        b4 = self.branch4(x)
        return self.project(torch.cat([b1, b2, b3, b4], dim=1))


class TinyDeepLabV3P(nn.Module):
    def __init__(self, in_channels: int = 1, base_channels: int = 16):
        super().__init__()
        c1, c3 = base_channels, base_channels * 4
        self.backbone = TinyBackbone(in_channels=in_channels, base_channels=base_channels)
        self.aspp = ASPP(in_channels=c3, out_channels=c1 * 2)
        self.low_proj = ConvBNReLU(c1, c1, kernel_size=1, stride=1, dilation=1)
        self.decoder = nn.Sequential(
            ConvBNReLU(c1 * 3, c1 * 2, kernel_size=3, stride=1, dilation=1),
            ConvBNReLU(c1 * 2, c1, kernel_size=3, stride=1, dilation=1),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        logits = self.decoder(torch.cat([aspp_up, low], dim=1))
        return logits
...
def train_deeplabv3p_track_o(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_val: np.ndarray,
    y_val: np.ndarray,
    cfg: DeepLabV3PTrackOConfig,
):
    ...
    model = TinyDeepLabV3P(in_channels=cfg.in_channels, base_channels=cfg.base_channels).to(device)
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("DeepLabv3+ Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_deeplabv3p_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
    ...
    torch.save(
        ...
        output_dir / "deeplabv3p_model.pt",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run DeepLabv3+ Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = (
        ...
        if args.output_dir
        else base / f"deeplabv3p_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    print(f"[done] deeplabv3p synthetic demo saved to: {out}")
...
def deeplabv3p_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "deeplabv3p")
    init_kwargs = filter_init_kwargs(TinyDeepLabV3P, {"in_channels": int(in_channels), **kwargs})
    model = TinyDeepLabV3P(**init_kwargs)
