...
class EfficientSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h: int, w: int) -> torch.Tensor:
        ...
        if self.sr is not None:
            ...
            x_img = x.transpose(1, 2).reshape(b, c, h, w)
            x_img = self.sr(x_img)
            kv = x_img.flatten(2).transpose(1, 2).contiguous()
            kv = self.norm(kv)
...
class TransformerEncoderBlock(nn.Module):
    def __init__(self, dim: int, num_heads: int, sr_ratio: int, mlp_ratio: float, dropout: float):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = EfficientSelfAttention(dim=dim, num_heads=num_heads, sr_ratio=sr_ratio, dropout=dropout)
        self.norm2 = nn.LayerNorm(dim)
        self.ffn = MixFFN(dim=dim, mlp_ratio=mlp_ratio, dropout=dropout)
...
class TinySegFormerEncoder(nn.Module):
    def __init__(
        self,
        in_channels: int = 1,
        embed_dims: Tuple[int, int] = (16, 32),
        ...
        self.patch1 = OverlapPatchEmbed(in_channels=in_channels, embed_dim=d1, patch_size=7, stride=2)
        self.block1 = TransformerEncoderBlock(
            dim=d1,
        ...
        self.patch2 = OverlapPatchEmbed(in_channels=d1, embed_dim=d2, patch_size=3, stride=2)
        self.block2 = TransformerEncoderBlock(
            dim=d2,
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        x1, h1, w1 = self.patch1(x)
        x1 = self.block1(x1, h1, w1)
        f1 = x1.transpose(1, 2).reshape(x1.shape[0], -1, h1, w1)

        x2, h2, w2 = self.patch2(f1)
        x2 = self.block2(x2, h2, w2)
        f2 = x2.transpose(1, 2).reshape(x2.shape[0], -1, h2, w2)
        return f1, f2


class SegFormerHead(nn.Module):
    def __init__(self, in_dims: Tuple[int, int], decoder_dim: int = 32, dropout: float = 0.1):
        super().__init__()
        self.proj1 = nn.Conv2d(in_dims[0], decoder_dim, kernel_size=1)
        self.proj2 = nn.Conv2d(in_dims[1], decoder_dim, kernel_size=1)
    ...
    def forward(self, f1: torch.Tensor, f2: torch.Tensor, out_hw: Tuple[int, int]) -> torch.Tensor:
        p1 = self.proj1(f1)
        p2 = self.proj2(f2)
        p2 = F.interpolate(p2, size=p1.shape[-2:], mode="bilinear", align_corners=False)
        logits_small = self.fuse(torch.cat([p1, p2], dim=1))
...
class TinySegFormer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.encoder = TinySegFormerEncoder(
        ...
        self.decode_head = SegFormerHead(in_dims=embed_dims, decoder_dim=embed_dims[1], dropout=dropout)
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        x_img = self._temporal_fusion(x_seq)
        f1, f2 = self.encoder(x_img)
        return self.decode_head(f1, f2, out_hw=(h, w))
...
def train_segformer_track_o(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_val: np.ndarray,
    y_val: np.ndarray,
    cfg: SegFormerTrackOConfig,
):
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("SegFormer Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_segformer_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SegFormer Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = (
        ...
        if args.output_dir
        else base / f"segformer_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    print(f"[done] segformer synthetic demo saved to: {out}")
...
def segformer_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "segformer")
    init_kwargs = filter_init_kwargs(TinySegFormer, {"in_channels": int(in_channels), **kwargs})
    model = TinySegFormer(**init_kwargs)
