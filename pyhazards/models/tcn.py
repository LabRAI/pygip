...
class TemporalConvNet(nn.Module):
    def __init__(self, in_channels: int, hidden_channels: int, kernel_size: int, num_levels: int, dropout: float):
        super().__init__()
        ...
        for i in range(num_levels):
            ...
            layers.append(TemporalBlock(cin, cout, kernel_size=kernel_size, dilation=dilation, dropout=dropout))
...
class TinyTCN(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        x = x.permute(0, 3, 4, 2, 1).contiguous()  # [B,H,W,D,T]
        x = x.reshape(b * h * w, d, t)  # [BHW,D,T]

        x = self.temporal(x)
        x_last = x[:, :, -1]  # [BHW,HID]

        x_last = x_last.reshape(b, h, w, self.hidden_channels).permute(0, 3, 1, 2).contiguous()
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("TCN Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_tcn_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run TCN Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = Path(args.output_dir) if args.output_dir else base / f"tcn_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"[done] tcn synthetic demo saved to: {out}")
...
def tcn_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "tcn")
    init_kwargs = filter_init_kwargs(TinyTCN, {"in_channels": int(in_channels), **kwargs})
    model = TinyTCN(**init_kwargs)
