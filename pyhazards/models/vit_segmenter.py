...
@dataclass
class ViTSegmenterTrackOConfig:
    seq_len: int = 6
    in_channels: int = 1
    patch_size: int = 4
    embed_dim: int = 64
...
class TinyViTSegmenter(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [TransformerBlock(embed_dim, num_heads, mlp_ratio=mlp_ratio, dropout=dropout) for _ in range(depth)]
        )
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        feat = self.patch_embed(x)  # [B,D,Hp,Wp]
        ...
        feat = tokens.transpose(1, 2).reshape(b, d, hp, wp).contiguous()
        logits_small = self.seg_head(feat)
        return F.interpolate(logits_small, size=(h, w), mode="bilinear", align_corners=False)
...
def train_vit_segmenter_track_o(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_val: np.ndarray,
    y_val: np.ndarray,
    cfg: ViTSegmenterTrackOConfig,
):
...
def save_history_and_plot(history: List[Dict[str, float]], output_dir: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("ViT Segmenter Track-O: train loss vs epoch")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def run_synthetic_demo(
    ...
    model, history, val_metrics, best_epoch, pos_weight = train_vit_segmenter_track_o(
        x_train,
        y_train,
        x_val,
        y_val,
    ...
    torch.save(
        ...
        output_dir / "vit_segmenter_model.pt",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ViT Segmenter Track-O synthetic smoke demo")
...
def main() -> None:
    ...
    out = (
        ...
        if args.output_dir
        else base / f"vit_segmenter_synthetic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    print(f"[done] vit_segmenter synthetic demo saved to: {out}")
...
def vit_segmenter_builder(task: str, in_channels: int = 1, out_dim: int = 1, **kwargs: Any) -> nn.Module:
    require_task(task, {"segmentation"}, "vit_segmenter")
    init_kwargs = filter_init_kwargs(TinyViTSegmenter, {"in_channels": int(in_channels), **kwargs})
    model = TinyViTSegmenter(**init_kwargs)
