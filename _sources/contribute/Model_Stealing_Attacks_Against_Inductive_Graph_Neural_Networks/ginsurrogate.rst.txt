...
Base class for Graph Isomorphism Network surrogate model implementation using DGL.
...
layers : nn.ModuleList
    List of GIN convolutional layers.
n_layers : int
...
evaluate_gin_surrogate(model, clf, g, inputs, labels, val_nid, batch_size, device)
    Evaluates surrogate model performance with classifier.

run_gin_surrogate(device, data, fan_out, batch_size, num_workers, num_hidden, num_layers, dropout, lr, num_epochs, log_every, eval_every)
