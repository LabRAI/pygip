...
``convgru_trajgru`` is the canonical PyHazards promotion of the wildfire benchmark Track-O baseline.
...
- Source implementation lineage: ``pyhazards.pipelines.wildfire_benchmark.models.convgru_trajgru_track_o``.
...
.. code-block:: python
   ...
   model = build_model(
       name="convgru_trajgru",
       task="segmentation",
   )

   if "segmentation" == "classification":
       x = torch.randn(4, 16)
   else:
